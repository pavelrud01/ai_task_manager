#!/usr/bin/env python3
"""
STEP-03: Enhanced Interview Collection
...
class EnhancedInterviewCollector:
    ...
    def collect_interviews(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Сбор интервью с ELICIT всех решений и детальным анализом Top-2
        """
        print("🎤 STEP-03: Enhanced Interview Collection...")
    ...
    def _conduct_enhanced_interview(self, interview_id: int, persona: str, product: str) -> Dict:
        """
        Проведение одного enhanced интервью
        """
    ...
    def _elicit_all_solutions(self, interview_id: int, persona: str, product: str, base_timestamp: str) -> Dict:
        ...
        """
        prompt = f"""
        Проведи ELICIT всех решений для AJTBD интервью.
        
        Персона: {persona}
        Продукт: {product}
        
        Задача: Собрать ВСЕ решения, которыми пользовался/пользуется респондент для достижения Big/Core гипотезы.
        ...
        - solutions: массив всех решений с деталями
    ...
    def _analyze_single_solution(self, interview_id: int, persona: str, product: str, 
        ...
        """
        prompt = f"""
        Проведи детальный анализ решения #{solution_rank} для AJTBD интервью.
        
        Персона: {persona}
        Продукт: {product}
        Решение: {solution.get('solution_name', 'N/A')}
        
        ОБЯЗАТЕЛЬНО собери ВСЕ поля:
        
        1. activation_knowledge: что «включает» действие
        2. psych_traits: психологические особенности
        3. prior_experience: прошлый опыт
        4. aha_moment: момент осознания ценности
        5. value_story: история ценности
        6. price_value_alignment: соответствие цены ценности (1-10)
        7. satisfaction: удовлетворенность (1-10) + почему
        8. cost: стоимость
        9. problems: проблемы (≥3-5) + follow-up по каждой: контекст/частота/серьезность/критический момент
        10. barriers: барьеры (≥3) + follow-ups
        ...
        - activation_knowledge: активирующее знание
        - psych_traits: психологические особенности
        - prior_experience: прошлый опыт
        - aha_moment: aha-момент
        - value_story: история ценности
        - price_value_alignment: соответствие цены ценности
        - satisfaction: удовлетворенность + обоснование
        - cost: стоимость
        - problems: массив проблем с follow-ups
        - barriers: массив барьеров с follow-ups
    ...
    def _create_report(self, interviews: List[Dict], validation_result: Dict[str, Any]) -> str:
        ...
        """
        report = f"""
# STEP-03 Report: Enhanced Interview Collection
...
3. STEP-04b: Longform export
    ...
    def _create_human_readable_version(self, interviews: List[Dict]) -> str:
        ...
        content = f"""# 🎤 ИНТЕРВЬЮ: Enhanced Collection

**Продукт:** Скорочтение (Matrius)  
**Дата:** {datetime.now().strftime('%d.%m.%Y')}  
**Модель:** gpt-5-high  
**Структура:** ELICIT → Top-2 Analysis → Classification → Completeness Check
...
**Структура:** Enhanced с ELICIT всех решений  
**Фокус:** Детальный анализ Top-2 решений
...
### 🎯 **ОСНОВНЫЕ РАБОТЫ:**
- Все интервью проведены по enhanced структуре
- Собраны ВСЕ решения респондентов
- Детально проанализированы Top-2 решения
- Работы классифицированы по типам и уровням

### 📅 **ХРОНОЛОГИЯ ПРОБЛЕМ:**
- Проблемы выявлены для каждого Top-2 решения
- Барьеры проанализированы с follow-ups
...
*Этот отчет основан на enhanced интервью с ELICIT всех решений и детальным анализом Top-2*
"""
...
def run_step_03_enhanced(config: Dict[str, Any], input_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Запуск STEP-03 с enhanced структурой
    """
    collector = EnhancedInterviewCollector(config)
